...
function printerError(s) {
    const notErrors = 'abcdefghijklm'.split('');
    const printerOutput = s.split('');
    let countErrors = printerOutput.filter(char => !notErrors.includes(char)).length;
    return `${countErrors}/${printerOutput.length}`;
}
